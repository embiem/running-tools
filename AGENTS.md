...
`running-tools` is a client-only Svelte 5 + TypeScript + Vite 7 SPA/installable PWA of small utilities for runners. Zero backend, zero runtime `dependencies` (everything is a devDependency). Currently one tool is implemented: a **cadence metronome** (Web Audio). Home page is a card grid; unimplemented tools render as placeholders.

## Architecture & Data Flow

- **Routing**: hand-rolled hash routing in `src/App.svelte` — no router library. A `routes` map (`'/'` → `src/pages/Home.svelte`, `'/metronome'` → `src/pages/MetronomePage.svelte`), `location.hash` read + `hashchange` listener, unknown routes fall back to `/`.
...
- **Data flow**: user input → Svelte runes state → engine mutation / `localStorage` → Web Audio scheduling. No network calls, no stores beyond Svelte built-ins.

## Key Directories

- `src/pages/` — route-level pages (Home grid, MetronomePage wrapper).
- `src/lib/` — reusable components and logic (`metronome.ts` engine, `Metronome.svelte` UI, `PWABadge.svelte` update toast).
...
- **Engine/UI separation**: platform logic lives in plain TS classes (`src/lib/*.ts`); `.svelte` files are thin bindings with scoped `<style>` blocks. Follow this split for new tools.
...
- **State persistence**: validated `localStorage` read + write in the same `$effect` that drives the engine (see `STORAGE_KEY` pattern in `Metronome.svelte`).
...
- If adding tests, the untested business logic target is `src/lib/metronome.ts` (pure scheduling logic); vitest fits the existing Vite toolchain and needs no new wiring beyond `tsconfig`.
